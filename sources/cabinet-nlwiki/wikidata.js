  ...
  return `SELECT DISTINCT (STRAFTER(STR(?item), STR(wd:)) AS ?wdid)
        ...
        WHERE {
          ...
          OPTIONAL {
            ...
            ?ref pr:P4656 ?source FILTER CONTAINS(STR(?source), 'nl.wikipedia.org') .
          ...
          OPTIONAL { ?item rdfs:label ?wdLabel FILTER(LANG(?wdLabel) = "nl") }
          BIND(COALESCE(?sourceName, ?wdLabel) AS ?name)

          OPTIONAL { ?positionItem wdt:P1705  ?nativeLabel   FILTER(LANG(?nativeLabel)   = "nl") }
          OPTIONAL { ?positionItem rdfs:label ?positionLabel FILTER(LANG(?positionLabel) = "nl") }
